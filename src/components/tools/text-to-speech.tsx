  ...
  const generate = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        try { const j = JSON.parse(t); throw new Error(j.error || `Server error (${res.status})`); } catch (e) { if (e instanceof SyntaxError) throw new Error(`Server error (${res.status}). Please try again.`); throw e; }
      }
